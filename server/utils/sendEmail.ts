...
const sendEmail = async (email: string): Promise<IResponse> => {
  const code: any = Math.random().toString().slice(2, 8)
  const html = readFileSync(join(__dirname, '../template/email/code.html'), { encoding: 'utf8' })
  const mailOptions = {
    from: config.nodemailer.user, // sender address
    to: email, // list of receivers
    subject: 'hi bro!', //主题是什么
    // text: '这是给用户用来获取验证码的,获取验证码输入验证码即可注册成功', //文本内容
    html: html.replace('{{code}}', code) //html模板
  }
